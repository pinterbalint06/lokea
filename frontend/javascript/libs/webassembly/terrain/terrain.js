...
var Wc = (a, b) => {
  ...
  return Ta(a, C, b, c);
}, fd = globalThis.TextDecoder ? new TextDecoder("utf-16le") : void 0, gd = (a, b, c) => {
  q(0 == a % 2, "Pointer passed to UTF16ToString must be aligned to two bytes!");
  a >>= 1;
  b = Pa(pa, a, b / 2, c);
  if (16 < b - a && fd) {
    return fd.decode(pa.subarray(a, b));
  }
  for (c = ""; a < b; ++a) {
  ...
  if (fa) {
    ...
    try {
      ...
      if (a = b, !(a instanceof Da || "unwind" == a)) {
        throw ha(), a instanceof WebAssembly.RuntimeError && 0 >= od() && r("Stack overflow detected.  You can try increasing -sSTACK_SIZE (currently set to 65536)"), a;
      }
    }
  }
}, qd = [], rd = [0, document, window], sd = a => {
  a = 2 < a ? N(a) : a;
  return rd[a] || document.querySelector(a);
}, W, td = 1, ud = [], X = [], vd = [], Y = [], wd = [], Z = [], xd = a => {
  ...
  Z[c] = b;
  return c;
}, Bd, Cd = ["default", "low-power", "high-performance"], Dd = {}, Fd = () => {
  if (!Ed) {
  ...
  return Ed;
}, Ed, Gd = a => "]" == a.slice(-1) && a.lastIndexOf("["), Hd = a => {
  a -= 5120;
...
var Id = {107808:() => {
...
}}, Oc = x("___getTypeName"), Jd = x("_malloc"), Qc = x("_free"), ia = x("_emscripten_stack_get_end"), Kd = x("__emscripten_timeout"), xb = x("_strerror"), Ld = x("_emscripten_stack_init"), od = x("_emscripten_stack_get_current"), wa = x("wasmMemory"), Mc = x("wasmTable"), Md = {__cxa_throw:(a, b, c) => {
  a = new Fa(a);
  w[a.m + 16 >> 2] = 0;
  w[a.m + 4 >> 2] = b;
  w[a.m + 8 >> 2] = c;
...
}, _embind_register_emval:a => P(a, ad), _embind_register_enum:(a, b, c, d) => {
  function e() {
  }
  b = R(b);
  e.values = {};
  ...
  }, B:(f, g) => g.value, P:bd(b, c, d), F:null});
  Bc(b, e);
  ...
  P(a, {name:b, o(c) {
    ...
    var g = Jd(4 + f + 1), k = g + 4;
    w[g >> 2] = f;
    e ? ed(d, k, f + 1) : C.set(d, k);
    null !== c && c.push(Qc, g);
    return g;
  }, P:dc, F(c) {
    Qc(c);
  ...
  if (2 === b) {
    ...
    q(4 === b, "only 2-byte and 4-byte strings are currently supported"), d = kd, e = ld, f = md;
  }
  P(a, {name:c, o:g => {
  ...
  nd[a] && (clearTimeout(nd[a].id), delete nd[a]);
  if (!b) {
    return 0;
  }
  var c = setTimeout(() => {
  ...
  e < f ? (ed(a, c, 17), ed(b, d, 17)) : (ed(a, d, 17), ed(b, c, 17));
  ...
  for (var d; d = C[b++];) {
    ...
    e &= 112 != d;
    c += e && c % 8 ? 4 : 0;
    qd.push(112 == d ? w[c >> 2] : 106 == d ? sa[c >> 3] : 105 == d ? G[c >> 2] : ra[c >> 3]);
    c += e ? 8 : 4;
  }
  q(Id.hasOwnProperty(a), `No EM_ASM constant found at address ${a}.  The loaded WebAssembly file is likely out of sync with the generated JavaScript.`);
  return Id[a](...qd);
  ...
  q(b);
  var c = b >> 2;
  b = {alpha:!!A[b + 0], depth:!!A[b + 1], stencil:!!A[b + 2], antialias:!!A[b + 3], premultipliedAlpha:!!A[b + 4], preserveDrawingBuffer:!!A[b + 5], powerPreference:Cd[G[c + 2]], failIfMajorPerformanceCaveat:!!A[b + 12], da:G[c + 4], Rb:G[c + 5], Nb:A[b + 24], cb:A[b + 25], Ub:G[c + 7], Wb:A[b + 32]};
  ...
  Z[a] = null;
}, emscripten_webgl_get_current_context:() => Bd ? Bd.handle : 0, emscripten_webgl_make_context_current:a => {
  Bd = Z[a];
  n.ctx = W = Bd?.$;
  return !a || W ? 0 : -5;
}, environ_get:(a, b) => {
  ...
  W.bindBufferRange(a, b, ud[c], d, e);
}, glBindTexture:(a, b) => {
  W.bindTexture(a, vd[b]);
}, glBindVertexArray:a => {
  ...
  for (var c = 0; c < a; c++) {
    ...
    e && (W.deleteBuffer(e), e.name = 0, ud[d] = null, d == W.Ha && (W.Ha = 0), d == W.ua && (W.ua = 0));
  }
}, glDeleteProgram:a => {
  if (a) {
    var b = X[a];
  ...
  for (var c = 0; c < a; c++) {
    ...
    e && (W.deleteTexture(e), e.name = 0, vd[d] = null);
  }
}, glDeleteVertexArrays:(a, b) => {
  for (var c = 0; c < a; c++) {
    var d = G[b + 4 * c >> 2];
    W.deleteVertexArray(wd[d]);
    wd[d] = null;
  }
  ...
  yd(a, b, "createBuffer", ud);
}, glGenTextures:(a, b) => {
  yd(a, b, "createTexture", vd);
}, glGenVertexArrays:(a, b) => {
  yd(a, b, "createVertexArray", wd);
  ...
  let f = Z[c].$, g = new Image(), k = N(b), l = $c(d), m = $c(e);
  g.onload = function() {
    let h = vd[a];
    h ? (f.bindTexture(f.TEXTURE_2D, h), f.texImage2D(f.TEXTURE_2D, 0, f.RGBA, f.RGBA, f.UNSIGNED_BYTE, g), f.generateMipmap(f.TEXTURE_2D), f.texParameteri(f.TEXTURE_2D, f.TEXTURE_MIN_FILTER, f.LINEAR_MIPMAP_LINEAR), f.texParameteri(f.TEXTURE_2D, f.TEXTURE_MAG_FILTER, f.LINEAR), f.bindTexture(f.TEXTURE_2D, null), "function" == typeof l && l()) : "function" == typeof m && m("Texture failed to load (it no longer exists):\t" + k);
  };
  g.onerror = function() {
    "function" == typeof m && m("Texture failed to load:\t" + k);
  };
  g.src = k;
}}, Nd;
function Od() {
...
ya = await (async function() {
  var a = {env:Md, wasi_snapshot_preview1:Md};
  ...
  od = a.emscripten_stack_get_current;
