...
function showError(message) {
    stopRoundTimer();
    document.getElementById('errorMessage').textContent = message || i18next.t("game:gamePage.unexpectedError", { defaultValue: "Váratlan hiba történt." });
...
async function startGame() {
    try {
        ...
        if (!Array.isArray(mapsData.maps) || mapsData.maps.length === 0) {
            ...
            throw new Error(i18next.t("game:gamePage.noMapsAvailable", { defaultValue: "Nincsenek elérhető térképek a játékhoz." }));
        }
        if (!gameData.game || typeof gameData.game.rounds !== 'number' || typeof gameData.game.roundTime !== 'number') {
            throw new Error(i18next.t("game:gamePage.invalidGameData", { defaultValue: "Érvénytelen játékadatok érkeztek a szervertől." }));
        }
        ...
        for (let i = currentRound; i < roundCount; i++) {
            ...
            await waitForGuess();
            if (isLastRound) {
                document.getElementById('nextRoundBtn').textContent = i18next.t("game:gamePage.resultsBtn", { defaultValue: "Eredmények" });
            }
            await waitForNext();
            removeEverything();
        }
        ...
        showError(i18next.t("game:gamePage.errorConnecting", { defaultValue: "Nem sikerült csatlakozni a játékhoz: " }) + error.message);
    }
}
...
async function createPoint(roundTime) {
    try {
        ...
        showError(i18next.t("game:gamePage.errorCreatingPoint", { defaultValue: "Hiba a pont létrehozásakor:" }) + " " + error.message);
    }
...
function nextMap() {
    ...
    loadMaplowThenHigh(mapId)
        .catch(error => showError(i18next.t("game:gamePage.errorLoadingMap", { defaultValue: "Hiba a térkép betöltésekor:" }) + " " + error.message));
}
...
async function sendGuess() {
    if (!guessSent) {
        ...
        try {
            ...
            showError(i18next.t("game:gamePage.errorSendingGuess", { defaultValue: "Nem sikerült elküldeni a tippet: " }) + error.message);
        }
    }
}
...
async function loadPointLowThenHigh(pId, markAsLoaded = () => { }) {
    ...
    try {
        ...
        throw new Error(error.message || i18next.t("game:gamePage.errorLoadingPointImage", { defaultValue: "Hiba a pont képének betöltésekor" }));
    }
}
...
async function loadMaplowThenHigh(mId) {
    try {
        ...
        throw new Error(error.message || i18next.t("game:gamePage.errorLoadingMapImage", { defaultValue: "Hiba a térkép képének betöltésekor" }));
    }
}
...
async function fetchGameData(url) {
    ...
    if (!response.ok) {
        ...
        throw new Error(responseData.error || i18next.t("game:gamePage.networkError", { defaultValue: "Hálózati hiba" }));
    }
...
async function finishGame() {
    ...
    try {
        ...
        showError(i18next.t("game:gamePage.errorFinishingGame", { defaultValue: "Hiba a játék befejezésekor: " }) + error.message);
    }
}
...
async function loadUserComment() {
    try {
        if (!gameMapId) {
            throw new Error(i18next.t("game:gamePage.noMapForComment", { defaultValue: "Nincs elérhető pálya a megjegyzés betöltéséhez." }));
        }
...
function showCommentFormState(editMode) {
    ...
    document.getElementById('submitRatingBtn').textContent = editMode ? i18next.t("game-maps:commentManager.saveChanges", { defaultValue: "Módosítás mentése" }) : i18next.t("game:gamePage.sendRating", { defaultValue: "Értékelés küldése" });
...
async function handleCommentSubmit() {
    ...
    try {
        if (commentIsSubmitting || !gameMapId) {
            throw new Error(i18next.t("game:gamePage.cannotSendRating", { defaultValue: "Nem lehet elküldeni az értékelést" }));
        }
        const rating = document.querySelector('[name="gameOverRating"]:checked')?.value;
        if (!rating) {
            showGameOverToast(i18next.t("game:gamePage.selectAtLeastOneStar", { defaultValue: "Kérlek válassz legalább 1 csillagot!" }), true);
            return;
        }
        ...
        if (!response.ok) {
            ...
            throw new Error(err.error || i18next.t("game:gamePage.errorOccurred", { defaultValue: "Hiba történt" }));
        }
        showGameOverToast(commentIsInEditMode ? i18next.t("game:gamePage.ratingUpdated", { defaultValue: "Értékelésed frissítve!" }) : i18next.t("game:gamePage.ratingSent", { defaultValue: "Értékelésed elküldve!" }));
        await loadUserComment();
        loadOtherComments();
    } catch (error) {
        showGameOverToast(error.message || i18next.t("game:gamePage.errorSendingRating", { defaultValue: "Hiba az értékelés elküldésekor" }), true);
    } finally {
...
async function handleDeleteComment() {
    try {
        if (commentIsSubmitting || !gameMapId) {
            throw new Error(i18next.t("game:gamePage.cannotDeleteRating", { defaultValue: "Nem lehet törölni az értékelést" }));
        }
        if (!confirm(i18next.t("game:gamePage.confirmDeleteRating", { defaultValue: "Biztosan törlöd az értékelésedet?" }))) {
            throw new Error(i18next.t("game:gamePage.deleteAborted", { defaultValue: "Törlés megszakítva" }));
        }
        commentIsSubmitting = true;
        ...
        showGameOverToast(i18next.t("game:gamePage.ratingDeleted", { defaultValue: "Értékelésed törölve." }));
        showCommentFormState(false);
        loadOtherComments();
    } catch (error) {
        showGameOverToast(error.message || i18next.t("game:gamePage.errorDeleting", { defaultValue: "Hiba a törléskor" }), true);
    } finally {
        commentIsSubmitting = false;
    }
}
...
async function loadLeaderboard() {
    ...
    try {
        if (!gameMapId) {
            throw new Error(i18next.t("game:gamePage.noMapForLeaderboard", { defaultValue: "Nincs elérhető pálya az eredménylista betöltéséhez." }));
        }
        const response = await fetch(`/api/game-maps/${gameMapId}`);
        if (!response.ok) {
            const err = await response.json();
            throw new Error(err.error || i18next.t("game:gamePage.errorLoadingLeaderboard", { defaultValue: "Hiba történt az eredménylista betöltésekor." }));
        }
        const data = await response.json();
        renderLeaderboard(container, data.game_map_details?.top_scores);
    } catch (error) {
        showGameOverToast(error.message || i18next.t("game:gamePage.errorLoadingLeaderboardToast", { defaultValue: "Hiba történt az eredménylista betöltésekor" }), true);
    }
}
...
function renderLeaderboard(container, topScores) {
    ...
    if (!topScores || topScores.length === 0) {
        ...
        empty.textContent = i18next.t("game:gamePage.noGamesYet", { defaultValue: "Még nincs játék ezen a pályán." });
...
function createLeaderboardItem(rank, scoreData) {
    ...
    time.textContent = `${i18next.t("game:gamePage.achievedAt", { defaultValue: "Elérve:" })} ${formatScoreTime(scoreData.score_time)}`;
...
async function loadOtherComments() {
    ...
    try {
        if (!gameMapId) {
            throw new Error(i18next.t("game:gamePage.noMapForComments", { defaultValue: "Nincs elérhető pálya a megjegyzések betöltéséhez." }));
        }
        const response = await fetch(`/api/game-maps/${gameMapId}/comments`);
        if (!response.ok) {
            const err = await response.json();
            throw new Error(err.error || i18next.t("game:gamePage.errorLoadingComments", { defaultValue: "Hiba történt a megjegyzések betöltésekor." }));
        }
        const data = await response.json();
        renderOtherComments(container, data.comments);
    } catch (error) {
        showGameOverToast(error.message || i18next.t("game:gamePage.errorLoadingCommentsToast", { defaultValue: "Hiba történt a megjegyzések betöltésekor" }), true);
    }
}
...
function renderOtherComments(container, comments) {
    ...
    if (!comments || comments.length === 0) {
        const empty = document.createElement('p');
        empty.className = 'gameover-comments-empty';
        empty.textContent = i18next.t("game:gamePage.noRatingsYet", { defaultValue: "Még nincsenek értékelések." });
        container.appendChild(empty);
    }
    else {
        const fragment = document.createDocumentFragment();
...
function createCommentItem(comment) {
    ...
    author.textContent = comment.username || i18next.t("game:gamePage.unknownUser", { defaultValue: "Ismeretlen" });
