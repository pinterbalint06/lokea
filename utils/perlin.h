...
void generatePerlinNoiseAnalytical(float *values, float *normals, float frequency, int size, uint32_t seed, float amplitude, int octaveCount, float lacunarity = 2, float persistence = 0.5, float noiseOffset = 0, float noiseSize = 1)
{
    ...
    std::fill(derivatives, derivatives + size * size * 2, 0.0f);
    for (int i = 0; i < octaveCount; i++)
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                derivatives[(y * size + x) * 2] += derivX * currAmplitude * toWorld;
                derivatives[(y * size + x) * 2 + 1] += derivY * currAmplitude * toWorld;
    ...
    for (int y = 0; y < size; y++)
    {
        for (int x = 0; x < size; x++)
        {
            ...
            derivatives[index * 2] *= normalizeNoiseCoeff;
            derivatives[index * 2 + 1] *= normalizeNoiseCoeff;
...
void generatePerlinNoiseFiniteDifference(float *values, float *normals, float frequency, int size, uint32_t seed, float amplitude, int octaveCount, float lacunarity = 2, float persistence = 0.5, float noiseOffset = 0, float noiseSize = 1)
{
    std::fill(normals, normals + size * size * 3, 0.0f);
    float currAmplitude = amplitude;
    float currFrequency = frequency;
