...
async function insertGameSession(userId, rounds, roundTime, gameMapId) {
    const query = 'INSERT INTO game_sessions (user_id, game_maps_id, rounds, time_per_round) VALUES (?, ?, ?, ?)';
    const [result] = await pool.execute(query, [userId, gameMapId, rounds, roundTime]);
    return result.insertId;
}

async function selectLatestActiveGameSession(userId) {
    const query = `
        SELECT game_maps.title, game_sessions.session_id, game_sessions.game_maps_id, game_sessions.current_cycle, game_sessions.rounds, game_sessions.current_round, game_sessions.time_per_round
        FROM game_sessions
