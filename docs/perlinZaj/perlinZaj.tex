...
meg, amikhez véletlenszerű vektorokat rendelünk. A gradiens tábla 256 darab
ilyen vektort tárol el. A vektorok dimenziószáma megegyezik a zaj
dimenziószámával. (Kétdimenziós zaj $\rightarrow$ kétdimenziós vektor)\\
\begin{minipage}[t]{0.6\textwidth}
    \vspace{0pt}
    \begin{algorithm}[H]
        \linespread{0.65}\selectfont
        \caption{Gradiens tábla létrehozása}
        \DontPrintSemicolon
        \Konstansok{MaxG=$256$}
        \Tipus{VéletlenSzámGenerátor=Osztály (\\
            ...
            \setlength\parindent{24pt} \textbf{Függvény} KövetkezőValós:Valós\\
            )}
...
\begin{minipage}[t]{0.7\textwidth}
    ...
    \begin{algorithm}[H]
        ...
        \Eljaras{veletlenVektorGeneral (\textbf{Változó}: Rand:VéletlenSzámGenerátor )}{
            \Valtozok{szog:Valós}
            \Valtozok{vektor:Vektor}
            szog $\coloneqq$ Rand.KövetkezőValós () * 2 * $\pi$\\
            vektor.x = $\cos(\text{szog})$\\
            vektor.y = $\sin(\text{szog})$\\
            \textbf{veletlenVektorGeneral} $\coloneqq$ vektor
        }
    \end{algorithm}
\end{minipage}
\hfill
...
\begin{algorithm}
    ...
    \Fuggveny{RacspontKiszamolasa(\textbf{Konstans}: $x$, $y$: Valós) : Rácspont}{
        ...
        \Komment{Pszeudókódban $1$-től kezdődik az indexelés}
        jelenlegiRácspont.balAlsóPontX $\coloneqq$ (((Egész)floor($x$)) \& $255$) + $1$ \\
        jelenlegiRácspont.balAlsóPontY $\coloneqq$ (((Egész)floor($y$)) \& $255$) + $1$ \\
        jelenlegiRácspont.jobbFelsőPontX $\coloneqq$ (jelenlegiRácspont.balAlsóPontX \& $255$) $+ 1$ \\ jelenlegiRácspont.jobbFelsőPontY $\coloneqq$
        (jelenlegiRácspont.balAlsóPontY \& $255$) $+ 1$ \\
        ~\\
...
\begin{algorithm}
    ...
    \Fuggveny{Simitofuggveny(\textbf{Konstans}: t: Valós) : Valós}{
        \Komment{Horner-elrendezésben ($12$ szorzása helyett csak $5$ szorzás):}
        \textbf{Simitofuggveny} $\coloneqq$ $t * t * t * (t * (6 * t - 15) + 10)$
    }
\end{algorithm}
...
amplitudóval rendelkezik.

\subsection{Paraméterei}
A fraktál zaj finomhangolható a következő paraméterekkel:

\begin{itemize}[noitemsep]
    \item \textbf{Oktávok:} Azt határozza meg, hány réteg zajt adunk össze. Minél magasabb ez a szám, annál részletesebb a végeredmény, de annál többször kell lefutattni a zajgenerálást.
    \item \textbf{Amplitudó (nagyság):} A zaj kezdeti magassága.
    ...
    \item \textbf{Persistence:} Azt határozza meg, hogy az oktávok között hogyan csökken a amplitudó. Az értéke általában $0.5$, tehát minden következő réteg fele olyan magas, mint az előző.
\end{itemize}

\subsection{FBM matematikailag és szemléltetése}
...
\]
Ahol az oktávok száma \textbf{$n$}, a kezdeti amplitudó \textbf{$A$}, a
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1]
        \begin{axis}[
            ...
            \addplot [domain=0:6.28, samples=200, color=black, thick]
            {1.0 * sin(deg(x)) + 0.5 * sin(deg(x * 2)) + 0.25 * sin(deg(x * 4)) + 0.125 * sin(deg(x * 8))};
            \addlegendentry{Összeg (FBM)}
        \end{axis}
    \end{tikzpicture}
    \caption{Az oktávok összegzésének szemléltetése 1 dimenzióban a szinuszfüggvénnyel.}
\end{figure}
\subsection{FBM pszeudókód}
A megvalósított kódban a fraktálzajt normalizáljuk a $[-1;1]$ intervallumra a
maximális lehetséges értékkel való osztással. A normalizálás után kettő saját
paramétert alkalmazunk.
\begin{itemize}[noitemsep]
    \item \textbf{Kontraszt:} A normalizált értéket erre az értékre emeli az eredeti előjel megtartásával. Így nagyobb lesz a kontraszt a nagyságok között.
    \item \textbf{Zajméret:} A kontraszt alkalmazása után egy adott értékkel megszorozza a zajt.
\end{itemize}
...
\begin{algorithm}
    ...
    \Fuggveny{FBM(\textbf{Konstans}: $x, y$: Valós) : Valós}{
        ...
        ~\\
        \Ciklus{$i \coloneqq 1$-tól $oktavok$-ig}{
            \Komment{Zaj hozzáadása az aktuális frekvenciával és amplitúdóval}
            zajErtek $\coloneqq$ zajErtek + Zaj($x \times jelenlegiFrekvencia$, $y \times jelenlegiFrekvencia$) $\times$ jelenlegiAmplitudo\\
            ~\\
            ...
            jelenlegiAmplitudo $\coloneqq$ jelenlegiAmplitudo $\times$ persistence\\
            jelenlegiFrekvencia $\coloneqq$ jelenlegiFrekvencia $\times$ lacunarity
        }
        ~\\
        \Komment{Normalizálás, kontraszt alkalmazása és méretezés}
        zajElojel $\coloneqq$ elojel(zajErtek) \\
        \textbf{FBM} $\coloneqq$ $(abszolutErtek(zajErtek / maxErtek))^{kontraszt} \times zajElojel \times zajMeret$
    }
\end{algorithm}
\newpage
